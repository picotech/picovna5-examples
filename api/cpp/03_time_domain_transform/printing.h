...
void printTimeDomainSample(const vna::TimeDomainSample& sample)
{
    /* Print this sample. */
    printf("%0.1f\t\t", sample.time * 10E9);
    printf("%0.3f", sample.sample);
    printf("\n");
}
...
// A helper function that just prints out the results of a time domain transform (time/sample value),
// for the purposes of the examples.
void printMeasurementPoints(const std::vector<vna::TimeDomainSample>& points)
{
    printTimeDomainHeader();
    for (const auto& p : points)
    {
        printTimeDomainSample(p);
    }
}
